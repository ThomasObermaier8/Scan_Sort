...
def run_scan(
    ...
    base_cmd = [naps2_path, "scan", "--output", str(output_target)]
    if profile.strip():
        base_cmd.extend(["--profile", profile.strip()])
    else:
        base_cmd.extend(["--driver", driver.strip()])
        if device.strip():
            base_cmd.extend(["--device", device.strip()])

    source_map = {"adf": "feeder", "glasplatte": "glass"}
    if source not in source_map:
        raise ValueError(f"Ungültige Quelle: {source}. Erlaubt sind: {', '.join(source_map)}")

    attempts: List[List[str]] = []
    if source == "adf":
        attempts.append(base_cmd + ["--source", source_map[source], "--duplex"])
    attempts.append(base_cmd + ["--source", source_map[source]])
    attempts.append(base_cmd)

    scan_exts = {".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp"}
    last_proc: Optional[subprocess.CompletedProcess] = None
    last_cmd: List[str] = attempts[-1]

    def collect_pages() -> List[Path]:
        return sorted(p for p in batch_dir.iterdir() if p.is_file() and p.suffix.lower() in scan_exts)
    ...
    for cmd in attempts:
        ...
        unknown_option = any(
            ...
            for text in [
                "unknown option",
                "unrecognized option",
                "option 'duplex' is unknown",
                "option 'source' is unknown",
                "'duplex' is unknown",
                "'source' is unknown",
            ]
        )
        # Bei unbekannter Option nächsten, einfacheren Versuch probieren.
        if unknown_option:
            continue
        # Bei echten Fehlern ebenfalls fallback versuchen; finaler Fehler kommt unten.
        continue

    assert last_proc is not None
    raise RuntimeError(
        "NAPS2-Scan fehlgeschlagen bzw. keine Bilddateien erzeugt.\n"
        f"Batch-Ordner: {batch_dir}\n"
        f"Erwartete Ziel-Datei: {output_target}\n"
        f"Letztes Kommando: {' '.join(last_cmd)}\n"
        f"Returncode: {last_proc.returncode}\n"
        f"STDOUT: {last_proc.stdout.strip()}\n"
        f"STDERR: {last_proc.stderr.strip()}"
    )
